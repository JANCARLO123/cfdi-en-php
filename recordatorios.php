...
require("t/Services/Twilio.php"); 
	    ...
	    {
	    $mensajeMovil = $row['name'].' desea que este día este lleno de sorpresas para tí, Feliz Cumpleaños!.';	
	    }elseif($row['tipo'] == 3) //Cumpleaños
	    {
	    /*Cambiar dominio.com/voice/ por el dominio real de la página*/	
	    $mensajeMovil = 'Has recibido un recordatorio de voz de '.$row['name'].' Consultalo en dominio.com/voice/'.$row['recurso'];	
...
{
/*	
 * Para agregar Twilio quitar el comentario de estas lineas
 * 
	$account_sid = 'id_de_la_cuenta';        //Id de la cuenta que da Twilio
    $auth_token = 'token_de_autenticacion';  //Token que da Twilio
	
	 try {
    $client = new Services_Twilio($account_sid, $auth_token); 
    $client->account->messages->create(array(  
    'To' => "+52".$row['telefono'], 
	'From' => "telefono",    //Teléfono que da Twilio
	'Body' => $mensajeMovil,  
      )); 
 ...
 * 
 * Estas líneas decrementan el número de recordatorios disponibles para el usuario
 * 
...
}else
{
	     ...
	     /* Cambiar dominio.com por el dominio real */
	     $subject = $row['name'].' te ha enviado un aviso por dominio.com'; 
         
		 /*Cambiar correo@dominio.com por el correo real del dominio */
