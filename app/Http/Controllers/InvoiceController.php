...
/*    $mensaje = preg_replace(array_keys($utf8), array_values($utf8), $mensaje);  
		   ...
		   try {
          $account_sid = 'id' Cambiar por el id real de twilio; 
          $auth_token = 'token'; Cambiar por el token real de twilio
	
          $client = new Services_Twilio($account_sid, $auth_token); 
          $client->account->messages->create(array(  
          'To' => "+52".$user -> telefono, 
	      'From' => "+52", Poner el teléfono rentado de Twilio    
	      'Body' => $mensaje,  
           ));
	...
	->attachData($pdf->output(), "factura.pdf");});
		...
		$mensaje = "Ha recibido una factura de ".$factura->emisor." por un monto de ".$factura->total_facturado." con el UUID ".$factura->uuid.". Buena dia. dominio.com"; //Cambiar dominio.com por dominio real
...
/*    $mensaje = preg_replace(array_keys($utf8), array_values($utf8), $mensaje);  
		   ...
		   try {
          $account_sid = 'id';    Cambiar por id de twilio
          $auth_token = 'token';  Cambiar por token de twilio
	
          $client = new Services_Twilio($account_sid, $auth_token); 
          $client->account->messages->create(array(  
          'To' => "+52".$user -> telefono, 
	      'From' => "+52",  Poner el teléfono real rentado en Twilio  
	      'Body' => $mensaje,  
           ));
